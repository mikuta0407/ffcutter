...
func main() {
	...
	flag.StringVarP(&opt.startTimeString, "start", "s", "", "start time (h:mm:ss)")
	flag.StringVarP(&opt.endTimeString, "end", "e", "", "end time (h:mm:ss)")
	...
	// 時間パース timeparse.go
	startSec, endSec, err := timeStringToSeconds(opt.startTimeString, opt.endTimeString)
	if err != nil {
		fmt.Println(err)
		os.Exit(1)
	}
	...
	if err := execFFmpegCommand(opt.inputFileName, opt.outputFileName, startSec, endSec, opt.audioOnly, opt.dryrun); err != nil {
		fmt.Println(err)
		os.Exit(1)
	}
}
